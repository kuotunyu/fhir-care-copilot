...
1. **看資料流與安全邊界**：[README 架構](../README.md#架構) → [README 安全邊界](../README.md#安全邊界)。
...
5. **看可重現的交付證據**：[GitHub Actions run 30792959630, attempt 2](https://github.com/kuotunyu/fhir-care-copilot/actions/runs/30792959630/attempts/2) → [公開 mock demo](https://huggingface.co/spaces/steven0226/fhir-care-copilot)。公開頁面應以 `/api/health` 回傳的 `provider=mock`、`model_id=mock-deterministic` 與 `demo_mode=true` 確認目前確實在無付費呼叫的 demo mode。
