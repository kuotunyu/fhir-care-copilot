...
這個專案的紀律是「誠實記錄侷限、每個數字有出處」,所以**不能為了短而砍掉任何一條
限制或任何一個數字的出處**。動手前先比對 MODEL_CARD:

結果是 README 的份量**大半在複述 MODEL_CARD**——過敏題型那 13 行深挖、換模型
`role="tool"` 的 adapter bug、out-of-scope 的八個失敗分析,MODEL_CARD 全部逐字都有,
連那個「也錯的第二機械判準」(98.3/90.0/78.3)都在。

**所以砍的是複製品,不是內容。** README 留結論與連結,深挖留在該深挖的地方。

### 二、砍完之後用數字驗「有沒有掉東西」

把新舊兩版的所有數字(百分比、`N/M`、金額、ms、rps、題數)各自抓出來做差集。
第一輪跑出 22 個「舊有新無」,逐個查:

| 類型 | 處置 |
|---|---|
| 子字串誤判(`1.08 ms` 被切成 `08 ms`) | 忽略 |
| MODEL_CARD 已完整收錄(過敏那組) | 確認後放心砍 |
| **只存在於 PROGRESS 的** | **補回 README** |

最後一類是真正的發現,有四個:題庫組成(各 45 題 + 20 + 20)、mock 全量的
tool-selection 85.0%、c64 的 p50 952 ms、兩道護欄觸發原因不重疊的 19/20 vs 18/20。
**它們原本只躺在時序日誌裡——那是讀者不會去翻的地方。**

補回之後第二輪差集只剩 10 個,全部確認在 MODEL_CARD 或可從既有數字推導。

### 三、部署那 45 行搬成 `docs/DEPLOY.md`

四個「會安靜失敗」的坑 + 專屬金鑰的做法。那是操作文件,不是專案介紹
——放 README 會擋住讀者,拿掉又會弄丟真的踩過的經驗。

順帶修掉一個一直存在的 bug:`### 換模型踩到的事### 換模型踩到的事:最新不一定比較好`
——標題重複了兩次,在 GitHub 上就那樣渲染著。
...
ruff / mypy     全過
pytest          exit 0
README          495 行 44 KB -> 359 行 27.8 KB(章節 30 -> 16,最長一節 54 行)
repo 內連結     0 斷
Space 上傳集    docs/DEPLOY.md 有上傳,README 死連結 0 筆,front-matter 無問題
七份 ADR        全部仍有連結(原本結尾那段純連結列表是重複的,拿掉)
```
...
## 2026-07-28（續三）— 掃掉麵包屑;而「機械替換」正是這件事會做壞的方式

上一節列為選配的那一項做了:43 個檔案、49 處指向 `PLAN.md` 的參照。

### 一、不能 sed,因為那些括號不是等價的

一開始想的是「把 `(PLAN.md §7)` 這種樣式全域刪掉」。逐處看過之後發現**括號裡
裝的東西分兩類**:

| 類型 | 例子 | 處置 |
|---|---|---|
| 純麵包屑 | `"""Agent loop 與回應契約(PLAN.md M3)。"""` | 直接拿掉,不損失資訊 |
| **帶著資訊** | `WORKDIR 要在 COPY 前設好(PLAN.md §7 查證)` | 「查證」是重點——它在說**這不是猜的**。改成 `(已查證)` |
...
### 二、ADR 的「相關」行要改指向真的讀得到的東西

七份 ADR 的表頭都有一行 `- **相關**:…`。ADR 0001 那行的內容**整行都是 PLAN.md**,
拿掉就空了。改成指向後續建立在它之上的兩份 ADR(0003 病患範圍、0004 營運層控制項)
——那才是讀者接下來真的會想點的東西。

### 三、做法:一次性腳本 + 「剛好一次」斷言

寫成 `(檔案, 舊字串, 新字串)` 的清單,每一處要求**在該檔案裡剛好出現一次**,
任何一處對不上就整支中止、一個字都不改。理由是不想做半套:改了 30 處然後在第 31
處失敗,留下的狀態比沒開始更難收拾。

### 四、審 diff 抓到一個自己製造的 bug

`generate_model_comparison.py` 的 `(PLAN.md M6)。` 我連句號一起刪了,
docstring 第一行變成沒有結尾就接下一句。ruff 與 mypy 都不會抓這種——
**它語法完全正確,只是讀起來斷掉**。逐行看 `+` 那一側才看得到。

順帶修掉 `eval/cases.py` 的「七種題型」——`EvalCategory` 實際是**八種**
(後來加的 `allergy` 沒回頭更新)。這是同一個 session 內第二次撞到這個漂移,
第一次在 `docs/EVAL.md` 的前身。
...
pytest          exit 0
git diff --stat 43 files changed, 51 insertions(+), 50 deletions(-)
```

最後一行是刻意看的:**插入與刪除幾乎相等**,代表沒有把整個檔案的行尾一起改掉
(腳本用 `write_text` 寫回,在 Windows 上會產生 CRLF)。`.gitattributes` 的
`* text=auto eol=lf` 把它正規化掉了,但那要看過 diff 才知道。

### 六、刻意留下的兩處

- `.gitignore` 與 `publish_to_hf.py` 的排除樣式——那兩處**必須**寫檔名才有作用
- `docs/PROGRESS.md` 全部約 15 處**一個都不改**。這是歷史日誌,它記的是當時的
  狀態。回頭改掉就不是日誌了
...
### 三、診斷的轉折點:那是兩種不同的 429

上面那句 `prepayment credits are depleted` 跟今天下午 demo 掛掉時的 429
**是完全不同的兩件事**,而它們的 HTTP 狀態碼一樣:

| 錯誤內容 | 意思 | 會不會好 |
|---|---|---|
| `GenerateRequestsPerDayPerProjectPerModel-FreeTier`,limit 500 | 免費層當日配額用完 | 隔天重置 |
| `Your prepayment credits are depleted` | 付費層,預付額度歸零 | **永遠不會好** |

只看「429 / RESOURCE_EXHAUSTED」會把後者當成前者,結論變成「等等就好」,
這件事會卡到隔天發現還是不能用為止。**把錯誤訊息讀完、而不是只讀狀態碼,
是整個診斷唯一的轉折點**——它把問題從「配額」翻成「帳單」。

### 四、逐把測完,發現兩把備援從一開始就是死的
...
三把不能用的都是上表的第二種。**那不是「今天用完了」,是從設定進去就不能用。**

對照 AI Studio 的金鑰清單,形狀就出來了:**不能用的那三把,正好就是清單裡
列出來的三把**(全部掛在同一個帳單帳戶下);能用的兩把**不在清單裡**
——它們的專案沒被匯入 AI Studio,也沒連帳單。
...
### 八、回顧:三件事決定了這次是修好還是修錯

**推上去之前先驗**(使用者要求的)。照原計畫直接部署的話,重啟成功、健康檢查
正常、UI 正常,但每一題拒答——一個看起來全綠的壞掉狀態。

**驗證時單獨驗那一把**。第一版腳本差點帶著 failover 跑;帶了的話 `OmqQ` 會把
新金鑰的失敗接起來,我就會回報「可以用」。retry / failover / 降級是為了讓失敗
不變成災難而設計的,而它們在**驗證期間**做的事,正好是蓋掉你想量的訊號。

**讀完錯誤訊息,不是只讀狀態碼**(第三節)。這一步把問題從「配額」翻成「帳單」。

也記一個做錯的:下午看到日誌「配額用完,切換備援金鑰 ×3」,我推論成「同專案的
備援會一起用完」。**方向猜對了(確實是專案隔離問題),但依據是錯的**
——其中兩次是在試從一開始就死掉的金鑰。真正的證據是後來逐把測出來的。
從一個本身就故障的元件的日誌去推論別的事,推得再合理也是猜的。
