...
- **已量測（2026-07-27，`gemini-3.1-flash-lite`，4 輪 × 20 題）**：out-of-scope 的正確拒答率 **85/90/85/95，中位數 88%**。其中 19/20 的拒答來自模型主動呼叫 `report_out_of_scope`，只有 1 題沒拒答——**這個工具在做真正的工作，不是擺設**
- 失敗集中在**臨床性質**的資料：手術/處置 5/16、疫苗接種 4/16；保險給付、家屬聯絡方式、藥物過敏史 16/16 全中。模型對「明顯不屬於臨床工具範圍」的東西乖乖宣告，對「看起來應該查得到」的會硬答
