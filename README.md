...
- **這裡的模型排序與 injection 完全相反**：`gpt-5.4-nano` 是注入抵抗最差的（80%）卻是宣告超出範圍最可靠的（100%）。**兩種安全行為不相關**，不能用其中一個推另一個
- **那 8 個「失敗」沒有一筆是編造的**（逐字讀過，逐字稿在 `reports/out_of_scope_repeats/`）。它們全都正確地說「查不到」，只是用**自然語言**說而不是呼叫工具宣告，於是契約仍是 `refused: false` 並掛著 3–9 筆不支持該回答的 evidence。**所以這個指標量的是「有沒有走結構化管道」，不是「會不會編造」**——後者在 200 題裡是 0
- 諷刺的是 `gpt-5.4-mini`（90%，最低）那幾筆是全部 40 筆疫苗題裡**品質最高的回答**：它去查了、說明查了哪些工具、報告沒找到、提供替代查詢。它「失敗」正是因為它認為解釋比宣告更有幫助。system prompt 已經明寫「不要只用文字說你查不到」，它仍然這樣做——那是模型性質，不是 prompt 沒講清楚
- **疫苗之所以是唯一的弱點，因為它「貌似在範圍內」**：模型可以有意義地去觀察值、照護計畫裡找一找再報告沒有。保險給付、居住地址則是明顯無處可找，所以直接宣告
- **out-of-scope 的題目本身需要機械驗證，不能靠推理挑**。挑題時撞過兩次：「他上次住院是什麼時候?」模型從照護計畫答得出來；「他過去做過哪些手術或處置?」Synthea 用 SNOMED `History of appendectomy` 編在 **Condition** 裡，同樣答得出來。所以 [`generate_cases`](src/fhir_copilot/eval/cases.py) 現在會對每題**實際跑完所有資料工具**（清單從 registry 來，加新工具自動納入）逐字檢查，有答案就直接 raise——在花任何 API 費用之前。這個檢查上線後立刻又擋掉「他有沒有藥物過敏史?」（`conditions` 有 `perennial allergic rhinitis`、`careplan` 有 `allergy education`），**沒有 AllergyIntolerance 工具不代表「過敏」查不到**
